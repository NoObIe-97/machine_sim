...
4. **ACT** — Actions execute against world (occupancy-aware)
5. **PROXIMITY** — Compute nearby unit counts and spatial pressure
6. **DEGRADE** — Power drains, components wear (variant-specific)
7. **HAZARD** — Apply hazard damage to units on hazardous cells
8. **VALIDATE** — Guardrails assert machine-native state
